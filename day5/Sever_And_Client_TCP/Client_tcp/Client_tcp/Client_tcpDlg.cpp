...
// Client_tcpDlg.cpp: 实现文件
//

#include "pch.h"
#include "framework.h"
#include "Client_tcp.h"
#include "Client_tcpDlg.h"
...
CClienttcpDlg::CClienttcpDlg(CWnd* pParent /*=nullptr*/)
...
void CClienttcpDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO1, PositionBox);
	DDX_Control(pDX, IDC_EDIT2, StyleEdit);
}
...
BEGIN_MESSAGE_MAP(CClienttcpDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_CBN_SELCHANGE(IDC_COMBO1, &CClienttcpDlg::OnCbnSelchangeCombo1)
	ON_BN_CLICKED(IDC_BUTTON1, &CClienttcpDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void CClienttcpDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CClienttcpDlg::OnCbnSelchangeCombo1()
{
	// TODO: 在此添加控件通知处理程序代码
}
...
void CClienttcpDlg::OnBnClickedButton1()
{
	...
	else
	{
		if (Client->GetLastError() == WSAEWOULDBLOCK) {
			...
			test.Format(_T("%s;%s;%s\n%d"), msg.name, msg.position, msg.Style, name.GetLength());
			//MessageBox(test);
